...
# Args: <BMC_IP> <BMC_USER> <BMC_PASS_FILE> <LOG_FILE> <BMC_SSH_PORT> [JOURNAL_PROFILE]
# JOURNAL_PROFILE: "vrc2" for PLDM-based stack; default for Software.BMC.Updater stack
# =============================================================================
...
sshpass -f "${BMC_PASS_FILE}" ssh ${SSH_OPTS} "${BMC_USER}@${BMC_IP}" "${REMOTE_CMDS}" 2>&1 | tee -a "${LOG_FILE}"
